...
func NewSniffer(ctx context.Context, txPool TxPool, chain blockChain, chainConfig *params.ChainConfig, feeder mamoru.Feeder) *SnifferBackend {
	...
	sb.TxSub = sb.SubscribeTransactions(sb.newTxsEvent)
...
func (bc *SnifferBackend) SubscribeTransactions(ch chan<- core.NewTxsEvent) event.Subscription {
	return bc.txPool.SubscribeTransactions(ch, false)
}
...
func (bc *SnifferBackend) process(ctx context.Context, header *types.Header, txs types.Transactions) {
	if ctx.Err() != nil || !bc.sniffer.CheckRequirements(header.Number.Uint64()) {
		return
	}
